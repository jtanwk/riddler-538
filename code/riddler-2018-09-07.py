...
def buy_pack(num_cards, card_list):
...
def collect_cards(num_cards):
    ...
    cards_left = [x for x in range(1, num_cards + 1)]
    num_packs = 0
...
for i in range(num_simulations):
    card_runs.append(collect_cards(num_cards))

mean_packs = statistics.mean(card_runs)
sd_packs = statistics.stdev(card_runs)

print(f"After {num_simulations} simulations, all {num_cards} cards were collected in {mean_packs} +/- {sd_packs} packs.")
